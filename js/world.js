...
function scaleWorld(){
    let scale;
    scale = Math.min(window.innerWidth, window.innerHeight - 50) / mapSize;       // -10 for safety
...
export function updateVariables(){
    ...
    getComputedStyle(snakeHead).getPropertyValue("--top") === getComputedStyle(fruit).getPropertyValue("--top")){
        bodyElement.style.setProperty("--score", 
        String(parseInt(getComputedStyle(bodyElement).getPropertyValue("--score")) + 1));
        changeFruitPosition();
        snake.fruitGathered()
    }
}
